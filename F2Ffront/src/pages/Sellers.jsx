      ...
      <div className="container mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {commodities.map((item) => (
          ...
          >
            ...
            <div className="mt-3">
              <p className="text-xs text-gray-600">Seller: <span className="font-medium text-purple-700">{item.seller.name}</span></p>
              <p className="text-xs text-gray-600">{item.seller.email}</p>
              <p className="text-xs text-gray-600">{item.seller.mobile}</p>
            </div>
          </div>
        ))}
      </div>
