...
const CommodityModalBuy = memo(({ isOpen, onClose, commodity }) => {
            ...
            <div className="bg-white rounded-lg p-6 max-w-full mx-4 overflow-auto h-[90%]">
                ...
                <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-2">
                        <LeafIcon className={`w-8 h-8 ${getThemeColor()}`} />
                    ...
                    <button
                        onClick={onClose}
                        className="p-2 hover:bg-gray-100 rounded-full transition-transform transform hover:scale-105"
                        aria-label="Close"
                    >
                ...
                <div className="border-t border-gray-200 pt-4 space-y-4 text-gray-700">
                    ...
                    <div className="bg-gray-50 rounded-lg p-2 space-y-2">
                        <div className="font-semibold text-gray-800">
                            {commodity.buyer.name}
                        </div>
                        <div className="flex items-center gap-2">
                            <Phone className="w-4 h-4 text-gray-500" />
                            <a href={`tel:${commodity.buyer.mobile}`} className="text-blue-600 hover:underline text-sm">
                                {commodity.buyer.mobile}
                            </a>
                        </div>
                        <div className="flex items-center gap-2">
                            <Mail className="w-4 h-4 text-gray-500" />
                            <a href={`mailto:${commodity.buyer.email}`} className="text-blue-600 hover:underline text-sm">
                                {commodity.buyer.email}
                            </a>
                        </div>
                    </div>
                </div>
                ...
                {/* Close Button */}
                <button
                    className="mt-6 w-full bg-green-500 text-white rounded-lg px-4 py-3 font-medium hover:bg-green-600 transition-colors focus:outline-none focus:ring-2 focus:ring-green-300"
                    onClick={onClose}
                >
                    Close
                </button>
...
export default CommodityModalBuy;